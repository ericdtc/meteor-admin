    ...
    adminNewUser: function (doc) {
        ...
        if (Roles.userIsInRole(this.userId, ['admin'])) {
            ...
            return _.each(emails, function (email) {
                ...
                if (doc.sendPassword && (AdminConfig.fromEmail)) {
                    Email.send({
                        to: user.email,
                        from: AdminConfig.fromEmail,
                        subject: 'Your account has been created',
                        html:  'You\'ve just had an account created for '
                              + Meteor.absoluteUrl() + ' with password '
                              + doc.password
                    });
                }
                if (!doc.sendPassword) {
